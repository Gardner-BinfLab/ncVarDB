...
#filter by MAF
for file in common_nc_dbSNP151/*; do
    python3 filter_by_MAF_100K.py $file 5-20_percent_MAF_nc_dbSNP151.csv error.csv
done

#or filter by MAF without splitting
#python3 filter_by_MAF.py common_non-coding_dbSNP151.txt 5-20_percent_MAF_nc_dbSNP151.csv error.csv

#variants in error.csv are excluded
...
Rscript filter_to_match_freq.R 5-20_percent_MAF_nc_dbSNP151_within30K.csv $path_file_path ncVar_benign_full_data.csv

#shift by one all substiutions and deletions to match with pathogenic
python3 shift_positions.py ncVar_benign_full_data.csv tmp

#sort
head -1 tmp > ncVar_benign_full_data.csv
sed 1d  tmp | sort -k1,1V -k2,2V >> ncVar_benign_full_data.csv
rm tmp

#convert to ncVar format
Rscript convert_to_ncVar_format.R ncVar_benign_full_data.csv ncVar_benign.csv

mv ncVat_benign.csv ../data
