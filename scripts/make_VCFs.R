...
Pathogenic <- read.delim("ncVar_pathogenic.tsv")
...
Benign <- read.delim("ncVar_benign.tsv")
...
write.table(Ben_VCF, "benign_vcf.tsv", quote = F,row.names = F,sep = "\t")
write.table(path_VCF, "pathogenic_vcf.tsv", quote = F,row.names = F,sep = "\t")
