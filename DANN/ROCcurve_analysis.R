...
all_variants$score <- as.numeric(all_variants$score)
unscored_variants <- all_variants[is.na(all_variants$score),]
all_variants <- all_variants[!is.na(all_variants$score),]


pdf("DANN_ROCcurve.pdf")
plot.roc(all_variants$ncVar_type, all_variants$score, print.thres=TRUE, print.thres.col="red", print.auc=TRUE)
dev.off()

write.table(unscored_variants, "DANN_ROCcurve_error.tsv", sep="\t", quote=F, row.names=F)
